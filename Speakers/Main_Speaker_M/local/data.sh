...
Main_Speaker_M_B2_Greek-M-B2-03-14
Main_Speaker_M_B2_Greek-M-B2-30-01
Main_Speaker_M_B2_Greek-M-B2-36-14
Main_Speaker_M_C1_Greek-M-C1-01-10
Main_Speaker_M_C1_Greek-M-C1-15-22
Main_Speaker_M_C1_Greek-M-C1-27-23
Main_Speaker_M_C2_Greek-M-C2-01-04
Main_Speaker_M_C2_Greek-M-C2-24-01
Main_Speaker_M_C2_Greek-M-C2-51-12
Main_Speaker_M_Harvard_Greek-M-Harvard-05-19
Main_Speaker_M_Harvard_Greek-M-Harvard-18-12
Main_Speaker_M_Harvard_Greek-M-Harvard-29-12
EOF
    # ----------------------------------
    ...
    for subset in "${subsets[@]}"; do
        for spk in "${speakers[@]}"; do
            ...
            find "${audio_root}" -name "*.wav" | sort | while read -r wav_path; do
                ...
                base_text_id=$(echo "${filename}" | sed 's/[MF]-//')
    ...
    log "Shuffling data/train for unbiased split using Python"
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ] && [ "${text_format}" = phn ]; then
    ...
    for dset in "${train_set}" "${dev_set}" "${eval_set}"; do
        ...
        utils/validate_data_dir.sh --no-feats data/${dset}_phn
    done
fi
