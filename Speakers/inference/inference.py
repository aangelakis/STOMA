...
base_dir = "/home/alex/STOMA_22050/Text/" # <-- Change this to your actual base directory path where the text files are stored
...
if speaker == "M":
    ...
    model_ckpt = f"{path}/M1/valid.loss.ave_5best.pth"
    model_config = f"{path}/M1/config.yaml"
elif speaker == "M2":
    model_ckpt = f"{path}/M2/valid.loss.ave_5best.pth"
    model_config = f"{path}/M2/config.yaml"
elif speaker == "F":
    model_ckpt = f"{path}/Main_Speaker_F/valid.loss.ave_5best.pth"
    model_config = f"{path}/Main_Speaker_F/config.yaml"
elif speaker == "F1":
    model_ckpt = f"{path}/F1/valid.loss.ave_5best.pth"
    model_config = f"{path}/F1/config.yaml"
elif speaker == "F2":
    model_ckpt = f"{path}/F2/valid.loss.ave_5best.pth"
    model_config = f"{path}/F2/config.yaml"

wav_out_dir = f"mos_{speaker}_{checkpoint}"
